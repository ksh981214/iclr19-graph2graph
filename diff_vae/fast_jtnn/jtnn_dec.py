...
class JTNNDecoder(nn.Module):
    ...
    def forward(self, mol_batch, x_tree_vecs, x_mol_vecs):
        ...
        traces = [] # [[],...,[]]
        #mol_batch 안에 있는 tree하나하나 마다 머 알아서 추적
        #순서가 있어서 testing할 동안에도 decoding된 순서대로 그래프가 조립된다
        ...
        for t in xrange(max_iter): # iter 1번에 하나씩 확장
            prop_list = []
            batch_list = []
            ...
            for node_x, real_y, _ in prop_list:
                ...
                cur_nei = [h[(node_y.idx,node_x.idx)] for node_y in node_x.neighbors if node_y.idx != real_y.idx] #현재 neighbor
        ...
        # 정답을 주고 teaching forcing 방법으로 학습함.
        # pred_loss==word_loss, label prediction 관련 loss, decoding 할 때 label을 맞췄는가?
        # stop_loss==topo_loss, topological prediction 관련 loss, decoding 할 때 더 이상 확장 할 지 안할지를 맞췄는가?
